...
export default {
  ...
  created() {
    ...
    // 如果 projectId 存在，则存储到 localStorage
    // if (projectId) {
    //   localStorage.setItem("projectId", projectId);
    // } else {
    //   // 如果路由参数中没有 projectId，尝试从 localStorage 获取
    //   const storedProjectId = localStorage.getItem("projectId");
    //   if (storedProjectId) {
    //     this.projectId = storedProjectId;
    //   } else {
    //     console.error("projectId 缺失");
    //   }
    // }

    // 使用 projectId（无论是从路由参数还是 localStorage 获取的）
    ...
    list({ projectId, pageIndex }).then((response) => {
      ...
      for (var i = 0; i < attributeList.length; i++) {
        if (attributeList[i] == "年份") {
          for (var j = 0; j < response.data.build_year; j++) {
      ...
      this.$confirm("此操作将永久删除该项目, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          for (
            ...
            console.log(this.$refs.entityTable.selection[i].productCode);
